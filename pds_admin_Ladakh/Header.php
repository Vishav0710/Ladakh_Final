...
if (session_status() !== PHP_SESSION_ACTIVE) {
    ...
    session_set_cookie_params([
        'lifetime' => 0,
        'secure'   => $secure,   // require HTTPS in prod
        'httponly' => true,
        'samesite' => 'Strict'   // 'Lax' if you need cross-site POSTs
    ]);
    session_start();
}

/* Disallow caching of restricted UI (helps with back button artifacts) */
...
if (isset($_SESSION['last_seen']) && time() - $_SESSION['last_seen'] > 1800) {
    session_unset();
    session_destroy();
    header('Location: /Login.html?reason=timeout', true, 302);
    exit;
}
$_SESSION['last_seen'] = time();

/* Block anonymous users from seeing this header (even if it’s included) */
if (empty($_SESSION['user_id'])) {
    header('Location: /Login.html?reason=auth', true, 302);
    exit;
}

/* Optional: enforce roles that may access admin UI */
$role = $_SESSION['role'] ?? null;
// allow only admin/editor to see admin navigation; tweak as needed
if (!in_array($role, ['admin', 'editor'], true)) {
    http_response_code(403);
    exit('Forbidden');
}

/* If someone hits header.php directly via URL, you can either: 
   (A) treat it like a restricted page (keep as is), or 
   (B) make it *never* directly reachable by returning 404. 
   To use option B, uncomment the two lines below.
*/
// if (basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'])) {
//     http_response_code(404); exit;
// }

/* Provide $current_page after auth */
    ...
    <body>
        ...
        <div class="page-sidebar">
			...
			<ul class="x-navigation">
				<li class="xn-logo">
					<span style="font-size: 19px; text-align: center; margin: 10px;color:#fff">Ladakh State </span>
